...
<title>MA03 分数をたす・ひく 【確認テスト】</title>
<style>
/* ══════════════════════════════════════════
...
<style>
  ...
  .op-sign { font-size:clamp(1.4em,4vw,2em); font-weight:700; color:var(--text); padding:0 4px; }
</style>
</head>
<body>
...
<main class="g-container">
  ...
  <div class="instruction" style="border-left-color:var(--accent);">
    <strong>【確認テスト】</strong>　分子と分母を入力して「判定する」を押してください。全問正解で合格です！
  </div>
  ...
  <div class="nav-btns" style="margin-top:20px;">
    ...
    <a id="btn-next" href="../MA04/intro.html" class="btn-nav btn-success" style="display:none;">次の単元へ →</a>
  </div>

</main>

<script src="ma03_sets.js"></script>
<script>
  const GAS_URL  = 'https://script.google.com/macros/s/ここにURLを貼り付け/exec';
  const setIndex = Math.floor(Math.random() * MA03_SETS.length);
  const problems = MA03_SETS[setIndex].problems;
      ...
      area.innerHTML += `
        <div class="problem-card" id="card${i}">
          <div class="problem-head" style="flex-wrap:wrap; gap:14px;">
            ...
            <div class="frac-row">
              <span class="frac">
                <span class="f-num">${p.n1}</span><span class="f-bar"></span><span class="f-den">${p.d1}</span>
              </span>
              <span class="op-sign">${opSign}</span>
              <span class="frac">
                <span class="f-num">${p.n2}</span><span class="f-bar"></span><span class="f-den">${p.d2}</span>
              </span>
  ...
  function judgeAll() {
    ...
    problems.forEach((p, i) => {
      ...
      const opSign = p.type==='add'?'＋':'－';
      const calc   = p.type==='add'?p.n1+p.n2:p.n1-p.n2;
      ...
      expl.innerHTML = (isOk
        ...
        + `<div style="background:var(--warn-light);border:1.5px solid var(--warn);border-radius:10px;padding:12px 16px;">
             <div style="font-size:.85em;font-weight:700;color:var(--warn);margin-bottom:8px;">💡 解き方</div>
             <p style="margin:0 0 6px;font-size:.9em;">分母は <strong style="color:var(--primary);">${p.d1}</strong> のまま変えません。</p>
             <p style="margin:0;font-size:.9em;">分子だけ計算：<strong style="color:var(--accent);">${p.n1} ${opSign} ${p.n2} ＝ ${calc}</strong></p>
           </div>`;
    });
    ...
    const msg = isPassed?'完璧です！次の単元へどうぞ 🚀'
    ...
    if (isPassed) {
      ...
      // 進捗をlocalStorageに保存
      localStorage.setItem('gamba_prog_MA03', 'pass');
  ...
  function sendToGAS(correct, total) {
    ...
    fetch(GAS_URL,{
      ...
      body:JSON.stringify({
        ...
        unit_code:'MA03', unit_name:'分数をたす・ひく',
