...
<main class="g-container">
  ...
  <div class="instruction">
    <strong>【演習】</strong>　逆数にしてかけ算に変えて計算しましょう。答えが整数のときは分母に 1 を入力してください。
  </div>
...
<script src="ma11_sets.js"></script>
<script>
  const setIndex = Math.floor(Math.random() * MA11_SETS.length);
  const problems = MA11_SETS[setIndex].problems;
        ...
        <div class="problem-card" id="card${i}">
          <div class="problem-head" style="flex-wrap:wrap; gap:14px;">
            ...
            <div class="frac-row" style="flex-wrap:wrap;">
              <span style="font-size:clamp(1.3em,3vw,1.8em); font-weight:700; color:var(--primary);">${p.w}</span>
              <span class="eq-sign">÷</span>
              <span class="frac"><span class="f-num">${p.n}</span><span class="f-bar"></span><span class="f-den">${p.d}</span></span>
              <span class="eq-sign">＝</span>
  ...
  function makeExplain(p, isOk, hasInput) {
    let html = '';
    if (hasInput) {
      html += isOk
      ...
      html += `<div class="ans-badge visible" style="background:var(--primary-light);border-color:var(--primary);color:var(--primary);margin-bottom:10px;">正解：分子 <strong>${p.ans_n}</strong>　分母 <strong>${p.ans_d}</strong></div>`;
    }
    html += `
      <div style="background:var(--warn-light);border:1.5px solid var(--warn);border-radius:10px;padding:10px 14px;margin-bottom:8px;">
        <div style="font-size:.8em;font-weight:700;color:var(--warn);margin-bottom:6px;">① 逆数にしてかけ算に変える</div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:clamp(.9em,2vw,1.05em);">
          <span style="font-weight:700;">${p.w}</span>
          <span style="color:var(--accent);font-weight:700;">÷</span>
          <span class="frac-sm"><span class="f-num">${p.n}</span><span class="f-bar"></span><span class="f-den">${p.d}</span></span>
          <span>＝</span>
          <span style="font-weight:700;">${p.w}</span>
          <span style="color:var(--success);font-weight:700;">×</span>
          <span class="frac-sm">
            <span class="f-num" style="color:var(--accent);font-weight:700;">${p.d}</span>
            <span class="f-bar" style="background:var(--accent);"></span>
            <span class="f-den" style="color:var(--accent);font-weight:700;">${p.n}</span>
          </span>
        </div>
      </div>
      <div style="background:var(--primary-light);border:1.5px solid var(--primary);border-radius:10px;padding:10px 14px;">
        <div style="font-size:.8em;font-weight:700;color:var(--primary);margin-bottom:6px;">② ななめ約分 → 計算</div>
  ...
  function toggleExplain(i) {
    ...
    const hasInput = !isNaN(valN) && !isNaN(valD);
    const isOk = hasInput && valN===p.ans_n && valD===p.ans_d;

    inN.classList.remove('correct','wrong');
    inD.classList.remove('correct','wrong');
    if (hasInput) { inN.classList.add(isOk?'correct':'wrong'); inD.classList.add(isOk?'correct':'wrong'); }

    expl.innerHTML = makeExplain(p, isOk, hasInput);
