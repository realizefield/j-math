...
<main class="g-container">
  ...
  <div class="instruction" style="border-left-color:var(--accent);">
    <strong>【確認テスト】</strong>　全問正解で合格！答えが整数のときは分母に 1 を入力してください。
  </div>
  ...
  <div class="nav-btns" style="margin-top:20px;">
    ...
    <a id="btn-next" href="../../index.html" class="btn-nav btn-success" style="display:none;">🎉 単元一覧へ</a>
  </div>

</main>

<script src="ma12_sets.js"></script>
<script>
  const GAS_URL  = 'https://script.google.com/macros/s/ここにURLを貼り付け/exec';
  const setIndex = Math.floor(Math.random() * MA12_SETS.length);
  const problems = MA12_SETS[setIndex].problems;
  let startTime  = Date.now();

  function mixedFrac(w, n, d) {
    let html = '';
    if (w > 0) html += `<span style="font-size:clamp(1.3em,3vw,1.8em);font-weight:700;color:var(--warn);">${w}</span>`;
    html += `<span class="frac"><span class="f-num">${n}</span><span class="f-bar"></span><span class="f-den">${d}</span></span>`;
        ...
        <div class="problem-card" id="card${i}">
          <div class="problem-head" style="flex-wrap:wrap; gap:14px;">
            ...
            <div class="frac-row" style="flex-wrap:wrap;">
              ${mixedFrac(p.w1, p.n1, p.d1)}
              <span class="eq-sign">÷</span>
              ${mixedFrac(p.w2, p.n2, p.d2)}
              <span class="eq-sign">＝</span>
              <span class="frac-dual">
                <input type="number" id="inp${i}_n" class="f-input" inputmode="numeric" placeholder="?">
                <span class="f-bar"></span>
                <input type="number" id="inp${i}_d" class="f-input" inputmode="numeric" placeholder="?">
              </span>
            </div>
          </div>
  ...
  function judgeAll() {
    ...
    problems.forEach((p, i) => {
      const inN  = document.getElementById('inp'+i+'_n');
      const inD  = document.getElementById('inp'+i+'_d');
      const expl = document.getElementById('expl'+i);
      const valN = parseInt(inN.value,10);
      const valD = parseInt(inD.value,10);
      const isOk = valN===p.ans_n && valD===p.ans_d;

      inN.classList.remove('correct','wrong');
      inD.classList.remove('correct','wrong');
      if (isOk) { correct++; inN.classList.add('correct'); inD.classList.add('correct'); }
      else       { inN.classList.add('wrong');  inD.classList.add('wrong'); }

      const conv1 = p.w1 > 0 ? p.w1 * p.d1 + p.n1 : null;
      const fn1 = conv1 || p.n1;
      const fd1 = p.d1;

      let html = isOk
        ? `<div class="ans-badge visible" style="background:var(--success-light);border-color:var(--success);color:var(--success);margin-bottom:10px;">⭕ 正解！　分子 <strong>${p.ans_n}</strong>　分母 <strong>${p.ans_d}</strong></div>`
        : `<div class="ans-badge visible" style="margin-bottom:10px;">❌ 正解は　分子 <strong style="color:var(--accent);font-size:1.2em;">${p.ans_n}</strong>　分母 <strong style="color:var(--accent);font-size:1.2em;">${p.ans_d}</strong></div>`;

      if (conv1) {
        html += `<div style="background:var(--warn-light);border:1.5px solid var(--warn);border-radius:10px;padding:10px 14px;margin-bottom:8px;">
          <div style="font-size:.8em;font-weight:700;color:var(--warn);margin-bottom:4px;">① 帯分数 → 仮分数</div>
          <p style="font-size:.88em;margin:0;">${p.w1}と${p.n1}/${p.d1} → <strong>${fn1}/${fd1}</strong></p>
        </div>`;
      }

      html += `<div style="background:var(--warn-light);border:1.5px solid var(--warn);border-radius:10px;padding:10px 14px;margin-bottom:8px;">
        <div style="font-size:.8em;font-weight:700;color:var(--warn);margin-bottom:6px;">${conv1?'②':'①'} 逆数にしてかけ算</div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:clamp(.85em,2vw,1em);">
          <span class="frac-sm"><span class="f-num">${fn1}</span><span class="f-bar"></span><span class="f-den">${fd1}</span></span>
          <span>÷</span>
          <span class="frac-sm"><span class="f-num">${p.n2}</span><span class="f-bar"></span><span class="f-den">${p.d2}</span></span>
          <span>＝</span>
          <span class="frac-sm"><span class="f-num">${fn1}</span><span class="f-bar"></span><span class="f-den">${fd1}</span></span>
          <span style="color:var(--success);font-weight:700;">×</span>
          <span class="frac-sm">
            <span class="f-num" style="color:var(--success);font-weight:700;">${p.d2}</span>
            <span class="f-bar" style="background:var(--success);"></span>
            <span class="f-den" style="color:var(--success);font-weight:700;">${p.n2}</span>
          </span>
        </div>
      </div>
      <div style="background:var(--primary-light);border:1.5px solid var(--primary);border-radius:10px;padding:10px 14px;">
        <div style="font-size:.8em;font-weight:700;color:var(--primary);margin-bottom:4px;">${conv1?'③':'②'} 約分 → 計算</div>
        <p style="font-size:.88em;margin:0 0 6px;color:var(--text-sub);">${p.reduce_note}</p>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
          答え：
          <span class="frac-sm">
            <span class="f-num" style="color:var(--accent);font-weight:700;">${p.ans_n}</span>
            <span class="f-bar" style="background:var(--accent);"></span>
            <span class="f-den" style="font-weight:700;">${p.ans_d}</span>
          </span>
          ${p.ans_d===1 ? `<span>＝</span><span style="font-size:1.1em;font-weight:700;color:var(--accent);">${p.ans_n}</span>` : ''}
        </div>
      </div>`;
    ...
    const msg = isPassed ? '完璧！分数÷分数マスターです 🎉'
              : ratio>=0.75 ? '惜しい！逆数の手順をもう一度確認しよう。'
              : '解説を見て手順を確認してもう一度！';
    ...
    resultArea.innerHTML = `
      ...
      ${isPassed ? '<div style="margin-top:12px;font-size:.95em;color:#16a34a;font-weight:700;">✅ 合格！分数の計算は全部クリアです！</div>' : ''}`;

    if (isPassed) {
      document.getElementById('btn-next').style.display = 'inline-block';
    // 進捗をlocalStorageに保存
    localStorage.setItem('gamba_prog_MA12', 'pass');
      ...
      fetch(GAS_URL, {
        ...
        body: JSON.stringify({
          ...
          unit_code:'MA12', unit_name:'分数÷分数',
          correct_count:correct, total_count:total,
          is_passed:true, study_minutes:studyMins
        })
